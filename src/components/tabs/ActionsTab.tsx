...
export default function ActionsTab() {
  ...
  return (
    <Box>
      ...
      <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={5}>
        {actions.map((a) => (
          <Box
            ...
            boxShadow="md"
            borderRightWidth={4}
            borderRightColor={`${STATUS_COLORS[a.status]}.400`}
          >
            <Text fontWeight="bold">{clientName(a.clientId)}</Text>
            <Badge colorScheme={STATUS_COLORS[a.status]} mb={2}>
              {a.status}
            </Badge>
      ...
      <EditModal
        title={editId ? "עריכת השאלה" : "השאלה חדשה"}
        isOpen={isOpen}
        onClose={onClose}
        onSave={handleSave}
        loading={saving}
      >
        <VStack spacing={4}>
          ...
          <FormControl>
            <FormLabel>מנשא / Carrier</FormLabel>
            <Select
          ...
          <FormControl>
            ...
            >
              {(["open", "pending", "handled", "waiting_list", "closed"] as ActionStatus[]).map(
                (s) => (
          ...
          <FormControl>
            <FormLabel>נלקח מ / Taken From</FormLabel>
            <Select
          ...
          <FormControl>
            <FormLabel>קשר אחרון ע״י / Last Contact By</FormLabel>
            <Select
          ...
          <FormControl>
            <FormLabel>תאריך החזרה / Return Date</FormLabel>
            <Input
              type="date"
